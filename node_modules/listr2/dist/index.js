...
import { EOL, EOL as EOL2 } from 'os';
...
const ListrError = class extends Error {
...
const PromptError = class extends Error {
  constructor(message) {
    super(message);
...
let _a;
const colorette_default = createColors({
  useColor: ((_a = process.env) == null ? void 0 : _a.LISTR_DISABLE_COLOR) !== '1',
});
...
var _DefaultRenderer = class {
  ...
  isBottomBar(task) {
    const { bottomBar } = this.getTaskOptions(task);
    return (
  ...
  getSelfOrParentOption(task, key) {
    ...
    let _b;
    return (
      ((_a2 = task == null ? void 0 : task.rendererOptions) == null ? void 0 : _a2[key]) ??
      ((_b = this.options) == null ? void 0 : _b[key])
    );
  }
    ...
    for (const task of tasks) {
      if (task.isEnabled()) {
        if (task.hasTitle()) {
          if (
            !(
              tasks.some((task2) => task2.hasFailed()) &&
              !task.hasFailed() &&
            ...
            if (task.hasFailed() && this.getSelfOrParentOption(task, 'collapseErrors')) {
              ...
              output = [
                ...output,
                this.formatString(
                  `${task == null ? void 0 : task.title} ${this.getTaskTime(task)}`,
                  this.getSymbol(task),
                  level
                ),
              ];
            ...
            output = [
              ...output,
              this.formatString(task.title, colorette_default.red(figures.squareSmallFilled), level),
            ];
          }
        }
        ...
        if (task == null ? void 0 : task.output) {
          if ((task.isPending() || task.isRetrying() || task.isRollingBack()) && task.isPrompt()) {
            ...
            if (
              !((_b = (_a2 = this.bottomBar[task.id]) == null ? void 0 : _a2.data) == null
                ? void 0
                : _b.some((element) => data.includes(element))) &&
              !task.isSkipped()
            ) {
        ...
        if (
          ...
          if (
            (subtaskRender == null ? void 0 : subtaskRender.trim()) !== '' &&
  ...
  formatString(str, icon, level) {
    ...
    switch (this.options.formatOutput) {
      case 'truncate':
        parsedStr = str.split(EOL).map((s, i) => cliTruncate(this.indentMultilineOutput(s, i), columns));
        break;
      case 'wrap':
  ...
  getSymbol(task, data = false) {
    ...
    if (task.isPending() && !data) {
      return ((_a2 = this.options) == null ? void 0 : _a2.lazy) ||
        (this.getSelfOrParentOption(task, 'showSubtasks') !== false &&
          task.hasSubtasks() &&
          !task.subtasks.every((subtask) => !subtask.hasTitle()))
        ? colorette_default.yellow(figures.pointer)
        : colorette_default.yellowBright(this.spinner[this.spinnerPosition]);
    }
    if (task.isCompleted() && !data) {
      return task.hasSubtasks() && task.subtasks.some((subtask) => subtask.hasFailed())
        ? colorette_default.yellow(figures.warning)
        : colorette_default.green(figures.tick);
    }
    if (task.isRetrying() && !data) {
      return ((_b = this.options) == null ? void 0 : _b.lazy)
        ? colorette_default.yellow(figures.warning)
        : colorette_default.yellow(this.spinner[this.spinnerPosition]);
    }
    if (task.isRollingBack() && !data) {
      return ((_c = this.options) == null ? void 0 : _c.lazy)
        ? colorette_default.red(figures.warning)
        : colorette_default.red(this.spinner[this.spinnerPosition]);
    }
...
const DefaultRenderer = _DefaultRenderer;
    ...
    this.eventTypeRendererMap = {
      ...
      ['MESSAGE' /* MESSAGE */]: (task, event) => {
        if (event.data.error) {
          ...
          this.log(
            `${colorette_default.yellow(figures.arrowDown)}${title} [${colorette_default.yellow(`skipped${skip}`)}]`
          );
...
const SimpleRenderer = _SimpleRenderer;
                ...
                if (event.type === 'SUBTASK' /* SUBTASK */ && task.hasSubtasks()) {
                  ...
                  if (((_a2 = this.options) == null ? void 0 : _a2.logEmptyTitle) !== false || task.hasTitle()) {
                    if (task.isPending()) {
                      ...
                      this.logger.success(
                        taskTitle +
                          (((_b = this.options) == null ? void 0 : _b.showTimer) &&
                          ((_c = task.message) == null ? void 0 : _c.duration)
                            ? ` [${parseTaskTime(task.message.duration)}]`
                            : '')
                      );
                    }
                  }
...
const VerboseRenderer = _VerboseRenderer;
...
const Task = class extends Subject {
  ...
  async run(context, wrapper) {
    ...
    try {
      const retryCount =
        ((_a2 = this.tasks) == null ? void 0 : _a2.retry) && ((_b = this.tasks) == null ? void 0 : _b.retry) > 0
          ? this.tasks.retry + 1
          : 1;
      ...
      if ((_c = this.tasks) == null ? void 0 : _c.rollback) {
        ...
        if (
          ...
          (await assertFunctionOrSelf((_e = this.tasks) == null ? void 0 : _e.exitOnError, context)) !== false
        ) {
...
async function createPrompt(options, settings) {
  ...
  if (!Array.isArray(options)) {
    ...
    options = options.reduce((o, option) => [...o, Object.assign(option, { name: 'default' })], []);
  }
  options = options.reduce(
    (o, option) => [
      ...
      Object.assign(option, {
        stdout:
          this instanceof TaskWrapper ? (settings == null ? void 0 : settings.stdout) ?? this.stdout() : process.stdout,
...
var TaskWrapper = class {
  ...
  report(error, type) {
    ...
    this.task.message$ = {
      error: error.message ?? ((_a2 = this.task) == null ? void 0 : _a2.title) ?? 'Task with no title.',
    };
  }
  ...
  skip(message) {
    ...
    if (message) {
      this.task.message$ = {
        skip: message ?? ((_a2 = this.task) == null ? void 0 : _a2.title) ?? 'Task with no title.',
      };
    }
  }
...
var Listr = class {
  constructor(task, options, parentTask) {
    ...
    const renderer = getRenderer(
      ...
      (_a2 = this.options) == null ? void 0 : _a2.rendererFallback,
      (_b = this.options) == null ? void 0 : _b.rendererSilent
    );
