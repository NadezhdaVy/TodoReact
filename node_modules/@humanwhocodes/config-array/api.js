...
const createDebug = _interopDefault(require('debug'));
...
function doMatch(filepath, pattern, options = {}) {
  ...
  if (!matcher) {
    matcher = new Minimatch(pattern, { ...MINIMATCH_OPTIONS, ...options });
...
class ConfigArray extends Array {
  ...
  constructor(configs, { basePath = '', normalized = false, schema: customSchema, extraConfigTypes = [] } = {}) {
     ...
     * @private
     */
    this[ConfigArraySymbol.schema] = new objectSchema.ObjectSchema({ ...customSchema, ...baseSchema });
  ...
  getConfig(filePath) {
    ...
    this.forEach((config, index) => {
      ...
      if (universalFiles.length) {
        ...
        if (
          ...
          pathMatches(filePath, this.basePath, { files: nonUniversalFiles, ignores: config.ignores })
        ) {
          debug(`Matching config found for ${filePath}`);
          matchingConfigIndices.push(index);
          matchFound = true;
        ...
        if (
          ...
          pathMatches(filePath, this.basePath, { files: universalFiles, ignores: config.ignores })
        ) {
          debug(`Matching config found for ${filePath}`);
          matchingConfigIndices.push(index);
          return;
    ...
    finalConfig = matchingConfigIndices.reduce(
      (result, index) => this[ConfigArraySymbol.schema].merge(result, this[index]),
      {},
      this
    );
